...
const findPrinter = (info) => {
    ...
    Object.values(info).forEach((pl) => {
        ...
        //console.log(`Plate ${pl.plate}`, avail, per, missing, pl.colors.length);
        let num = 0;
        let ps = 'None';
                ...
                missing.forEach(m => {
                    ...
                    _m.filaments.forEach((f, idx) => {
                        if (f.type === type && f.color.indexOf('/') === -1) {
                            ...
                            //console.log(color, f.color, diff);
                            _colors.push({
                                color:  f.color,
                                type:   f.type,
                                fcolor: f.color,
                    ...
                    let inform = `[${m.id}] ${cfg.getMachine(m.id).name} missing: ${fil[0]} ${colorBox(fil[1])} #${fil[1]}`;
                    const guess = _colors[0];
                    if (guess) {
                        const amsID = amsTrayNumToLetters(guess.tray);
                        inform += ` best guess: ${guess.type} ${colorBox(guess.fcolor)} #${guess.fcolor} (${amsID})`;
                    } else {
