...
const showAll = (args, _machines) => {
    ...
    async.each(machines, statusFTP, () => {
        machines = JSON.parse(JSON.stringify(_machines));
        async.each(machines, statusMQTT, () => {
            logger.debug('statusCheck', JSON.stringify(statusCheck, null, 4));
    ...
    if (machine.ams && machine.ams.ams) {
        ...
        ams.forEach(a => {
            ...
            trays.forEach((tray) => {
                ...
                if (tray.cols && tray.cols.length > 1) { //Mulicolor
