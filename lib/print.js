...
module.exports = (args) => {
    ...
    if (!args.file || args.file === true) {
        console.error(`⛔ Please pass a file to start the print with. It needs to be uploaded first!`);
        process.exit(1);
    }
    ...
    console.log(`Sending print command to ${machine.id}`);
    ...
    client.on("connect", (e) => {
        ...
        client.end();
        setTimeout(() => {
            console.log(`Finished sending job to ${machine.id}, use status to check on it.`);
            status({});
        }, 1000);
    });
};
